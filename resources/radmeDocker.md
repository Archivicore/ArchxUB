...
- Setup configs
    - Download the sample config file <br>
        - ```mkdir Archx && cd Archx```
        - ```wget https://raw.githubusercontent.com/Archivicore/Archx/master/config.env.sample -O config.env```
        - ```vim config.env```
    - Download the yml file for docker-compose
        - ```wget https://raw.githubusercontent.com/Archivicore/Archx/master/resources/docker-compose.yml```
