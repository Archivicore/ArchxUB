...
_LOG = Archx.logging.getLogger(__name__)
...
def get_file_id_of_media(message: 'Archx.Message') -> Optional[str]:
    """ get file_id """
...
# https://github.com/ArchxTeam/Archx-Plugins/blob/master/plugins/anilist.py
def post_to_telegraph(a_title: str, content: str) -> str:
    ...
    post_page = post_client.post(
        title=a_title,
        author=auth_name,
        author_url="https://t.me/theArchx",
        text=content
    )
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    ...
    ttl = duration // 2
    thumb_image_path = path or join(Archx.Config.DOWN_PATH, f"{basename(video_file)}.jpg")
...
def is_command(cmd: str) -> bool:
    commands = Archx.Archx.manager.enabled_commands
    key = Archx.Config.CMD_TRIGGER + cmd
    _key = Archx.Config.SUDO_TRIGGER + cmd
