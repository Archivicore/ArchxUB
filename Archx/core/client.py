...
from Archx import logging, Config, logbot
from Archx.utils import time_formatter
from Archx.utils.exceptions import ArchxBotNotFound
from Archx.plugins import get_all_plugins
...
_Archx_STATUS = get_collection("Archx_STATUS")


async def _set_running(is_running: bool) -> None:
    await _Archx_STATUS.update_one(
        {'_id': 'Archx_STATUS'},
...
async def _is_running() -> bool:
    if Config.ASSERT_SINGLE_INSTANCE:
        data = await _Archx_STATUS.find_one({'_id': 'Archx_STATUS'})
        if data:
...
class _AbstractArchx(Methods, RawClient):
    @property
    def id(self) -> int:
        if self.is_bot:
    ...
    async def load_plugin(self, name: str, reload_plugin: bool = False) -> None:
        """ Load plugin to Archx """
        _LOG.debug(_LOG_STR, f"Importing {name}")
        _IMPORTED.append(
            importlib.import_module(f"Archx.plugins.{name}"))
        if reload_plugin:
    ...
    def __eq__(self, o: object) -> bool:
        return isinstance(o, _AbstractArchx) and self.id == o.id

    def __hash__(self) -> int:
        return super().__hash__()


class ArchxBot(_AbstractArchx):
    """ ArchxBot, the bot """
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting ArchxBot Configs")
...
class Archx(_AbstractArchx):
    """ Archx, the userbot """

    has_bot = bool(Config.BOT_TOKEN)

    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting Archx Configs")
        kwargs = {
        ...
        if Config.HU_STRING_SESSION and Config.BOT_TOKEN:
            ...
            kwargs['bot'] = ArchxBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['ArchxBot', 'Archx']:
        """ returns Archxbot """
        if self._bot is None:
            if Config.BOT_TOKEN:
                return self
            raise ArchxBotNotFound("Need BOT_TOKEN ENV!")
        return self._bot
    ...
    async def start(self) -> None:
        ...
        while await _is_running():
            _LOG.info(_LOG_STR, "Waiting for the Termination of "
                                f"previous Archx instance ... [{timeout} sec]")
            time.sleep(timeout)
        ...
        _LOG.info(_LOG_STR, "Starting Archx")
        await _set_running(True)
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting ArchxBot")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping ArchxBot")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping Archx")
        await super().stop()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(_sig: signal.Signals) -> None:
            ...
            _LOG.info(_LOG_STR, f"Received Stop Signal [{_sig.name}], Exiting Archx ...")
            await _finalize()
        ...
        logbot.edit_last_msg("Archx has Started Successfully !")
        logbot.end()
        ...
        try:
            if coro:
                ...
                _LOG.info(_LOG_STR, f"Idling Archx - {mode}")
                idle()
