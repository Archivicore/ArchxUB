...
@Archx.on_filters(filters.group & filters.new_chat_members, group=1, check_restrict_perm=True)
    ...
    for user in message.new_chat_members:
        ...
        if gbanned:
            ...
            if Config.Archx_ANTISPAM_API:
                try:
                    ban = await _get_Archx_antispam_data(user_id)
...
@pool.run_in_thread
def _get_Archx_antispam_data(user_id: int):
    return Client(Config.Archx_ANTISPAM_API).getban(user_id)
