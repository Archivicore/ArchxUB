...
"""
    ...
    if thumb:
        args = {
            'caption': out_str,
            'parse_mode': "md",
            'reply_markup': markup if Archx.has_bot else None
        }
    else:
        args = {
            'text': out_str,
    ...
    for chat_id in RSS_CHAT_ID:
        ...
        try:
            await send_rss_to_telegram(Archx.bot, args, thumb)
        except (
            ChatWriteForbidden, ChannelPrivate, ChatIdInvalid,
            UserNotParticipant, ArchxBotNotFound
        ):
            ...
            await send_rss_to_telegram(Archx, args, thumb)
