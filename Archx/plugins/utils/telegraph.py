...
from Archx.utils import post_to_telegraph
...
async def telegraph_(message: Message):
    ...
    if (replied.text
        ...
        if replied.document:
            ...
            if not header:
                header = "Pasted content by @theArchx"
            os.remove(dl_loc)
        else:
    ...
    if replied.sticker:
        ...
        img.save(f'{Config.DOWN_PATH}/Archx.png', 'png')
        os.remove(dl_loc)
        dl_loc = f'{Config.DOWN_PATH}/Archx.png'
