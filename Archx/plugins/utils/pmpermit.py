...
from Archx import Archx, filters, Message, Config, get_collection
from Archx.utils import SafeDict
...
@Archx.on_cmd("allow", about={
    'header': "allows someone to contact",
    'description': "Ones someone is allowed, "
                   "Archx will not interfere or handle such private chats",
...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await Archx.get_users(userid)).unblock()
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await Archx.get_users(user)).id
...
@Archx.on_cmd("setpmmsg", about={
    'header': "Sets the reply message",
    'description': "You can change the default message which Archx gives on un-invited PMs",
...
@Archx.on_filters(~allowAllFilter & filters.incoming & filters.private & ~filters.bot
                   & ~filters.me & ~filters.service & ~Config.ALLOWED_CHATS, allow_via_bot=False)
async def uninvitedPmHandler(message: Message):
    """ pm message handler """
    user_dict = await Archx.get_user_dict(message.from_user.id)
    ...
    if message.from_user.id in pmCounter:
        ...
        if Archx.has_bot and _IS_INLINE:
            try:
                bot_username = (await Archx.bot.get_me()).username
                k = await Archx.get_inline_bot_results(bot_username, "pmpermit")
...
@Archx.on_filters(~allowAllFilter & filters.outgoing & ~filters.edited
...
async def outgoing_auto_approve(message: Message):
    ...
    await ALLOWED_COLLECTION.update_one(
        {'_id': userID}, {"$set": {'status': 'allowed'}}, upsert=True)
    user_dict = await Archx.get_user_dict(userID)
    await CHANNEL.log(f"**#AUTO_APPROVED**\n{user_dict['mention']}")

if Archx.has_bot:
    @Archx.bot.on_callback_query(filters.regex(pattern=r"pm_allow\((.+?)\)"))
    async def pm_callback_allow(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await Archx.unblock_user(userID)
            user = await Archx.get_users(userID)
            if userID in Config.ALLOWED_CHATS:
    ...
    @Archx.bot.on_callback_query(filters.regex(pattern=r"pm_block\((.+?)\)"))
    ...
    @Archx.bot.on_callback_query(filters.regex(pattern=r"^pm_spam$"))
    async def pm_spam_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            user_dict = await Archx.get_user_dict(c_q.from_user.id)
            await c_q.edit_message_text(
                blocked_message.format_map(SafeDict(**user_dict)))
            await Archx.block_user(c_q.from_user.id)
            await asyncio.sleep(1)
            await CHANNEL.log(
                f"#BLOCKED\n{c_q.from_user.mention} has been blocked due to spamming in pm !! ")

    @Archx.bot.on_callback_query(filters.regex(pattern=r"^pm_contact$"))
