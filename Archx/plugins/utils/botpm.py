...
from Archx import Archx, Message, Config, filters, get_collection, pool
from Archx.utils import SafeDict, time_formatter
...
if Archx.has_bot:
    Archx_id = Archx.id if Archx.dual_mode else Config.OWNER_ID[0]
    bot = Archx.bot
    ...
    async def start(_, msg: PyroMessage):
        ...
        if user_id != Archx_id:
            ...
            copy_ = "https://github.com/ArchxTeam/Archx/blob/master/LICENSE"
            markup = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(text="👥 ArchxTeam", url="https://github.com/ArchxTeam"),
    ...
    @bot.on_message(filters.user(Archx_id) & filters.private & filters.command("settext"))
    ...
    @bot.on_message(filters.user(Archx_id) & filters.private & filters.command("pmban"))
    async def pm_ban(_, msg: PyroMessage):
        ...
        elif replied and replied.from_user.id == Archx_id:
    ...
    @bot.on_message(filters.user(Archx_id) & filters.private & filters.command("pmunban"))
...
**Outgoing:** `{_STATS["outgoing"]}`
            ...
            user_dict = await Archx.get_user_dict(cq.from_user.id)
    ...
    async def handle_incoming_message(msg: PyroMessage):
        ...
        try:
            if msg.sticker:
                await bot.send_message(Archx_id, f"{msg.from_user.mention} sent you a sticker.")
            m = await msg.forward(Archx_id)
    ...
    async def handle_reply(msg: PyroMessage):
        ...
        elif replied.from_user.id == Archx_id:
...
/cancel - cancel the current operation """
        ...
        temp_msgs = []
        async with Archx.bot.conversation(
