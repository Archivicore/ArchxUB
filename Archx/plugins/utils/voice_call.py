...
from Archx import Archx, Message, pool, filters, get_collection, Config
...
call = GroupCall(Archx, play_on_repeat=False)
...
def check_enable_for_all(func):
    ...
    async def checker(msg: Message):
        if msg.from_user.id == Archx.id or CMDS_FOR_ALL:
            await func(msg)
...
def check_cq_for_all(func):
    ...
    async def checker(_, c_q: CallbackQuery):
        if c_q.from_user.id == Archx.id or CMDS_FOR_ALL:
            await func(c_q)
        else:
            await c_q.answer(
...
@Archx.on_cmd("play", about={'header': "play or add songs to queue"},
...
@Archx.on_cmd("helpvc", about={'header': "help for voice_call plugin"},
               trigger=Config.SUDO_TRIGGER, allow_private=False, check_client=True,
               filter_me=False, allow_bots=False)
@vc_chat
@check_enable_for_all
async def _help(msg: Message):
    ...
    commands = Archx.manager.enabled_plugins["voice_call"].enabled_commands
    cmds = []
    raw_cmds = []
...
📘 <b>Doc:</b>  <code>Archx Voice-Call Plugin</code>\n\n"""
...
async def current(msg: Message):
    ...
    await reply_text(
        msg,
        BACK_BUTTON_TEXT,
        markup=default_markup() if Archx.has_bot else None,
        to_reply=True
    )


@Archx.on_cmd("queue", about={
...
async def stop_music(msg: Message):
    ...
    await reply_text(msg, "`Stopped Archx-Music.`", del_in=5)
...
if Archx.has_bot:
    @Archx.bot.on_callback_query(filters.regex("(skip|queue|back)"))
    @check_cq_for_all
    ...
    @Archx.bot.on_callback_query(filters.regex(r"vol\((.+)\)"))
    @check_cq_for_all
    async def vol_callback(cq: CallbackQuery):
        ...
        if arg.isnumeric():
            ...
            try:
                async with Archx.conversation(cq.message.chat.id, user_id=cq.from_user.id) as conv:
