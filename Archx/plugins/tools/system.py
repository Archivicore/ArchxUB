...
from Archx.core.ext import RawClient
from Archx import Archx, Message, Config, get_collection
from Archx.utils import terminate
...
async def restart_(message: Message):
    """ restart Archx """
    await message.edit("`Restarting Archx Services`", log=__name__)
    LOG.info("Archx Services - Restart initiated")
    ...
    if 'h' in message.flags:
        if Config.HEROKU_APP:
            ...
            asyncio.get_event_loop().create_task(Archx.restart(hard=True))
    else:
        await message.edit("`Restarting [SOFT] ...`", del_in=1)
        asyncio.get_event_loop().create_task(Archx.restart())


@Archx.on_cmd("shutdown", about={'header': "shutdown Archx :)"}, allow_channels=False)
...
async def die_(message: Message) -> None:
    """ set offline timeout to die Archx """
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@Archx.on_cmd("listdisabled", about={'header': "List all disabled chats."})
...
@Archx.on_cmd("sleep (\\d+)", about={
    'header': "sleep Archx :P",
...
async def _slp_wrkr(seconds: int) -> None:
    ...
    await Archx.reload_plugins()
    await Archx.start()


@Archx.on_user_status()
