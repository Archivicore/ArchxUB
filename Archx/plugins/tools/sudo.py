...
@Archx.on_cmd("sudo", about={'header': "enable / disable sudo access"}, allow_channels=False)
...
@Archx.on_cmd("vsudo", about={'header': "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if '-all' in message.flags:
        ...
        tmp_ = []
        for c_d in list(Archx.manager.enabled_commands):
...
@Archx.on_cmd("vscmd", about={'header': "view sudo cmds"}, allow_channels=False)
