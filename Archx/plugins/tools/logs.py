...
from Archx import Archx, Message, logging, Config, pool
...
@Archx.on_cmd("logs", about={
    'header': "check Archx logs",
    'flags': {
        '-h': "get heroku logs",
...
async def check_logs(message: Message):
    ...
    if '-h' in message.flags and Config.HEROKU_APP:
        ...
        await message.client.send_as_file(chat_id=message.chat.id,
                                          text=logs,
                                          filename='Archx-heroku.log',
                                          caption=f'Archx-heroku.log [ {limit} lines ]')
    else:
        await message.client.send_document(chat_id=message.chat.id,
                                           document="logs/Archx.log",
                                           caption='Archx.log')
    await message.delete()


@Archx.on_cmd("setlvl", about={
