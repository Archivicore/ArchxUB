...
async def status(message: Message) -> None:
    ...
    if not type_:
        out_str = f"""📊 **--Archx Status--** 📊

🗃 **Plugins** : `{len(Archx.manager.plugins)}`
        ✅ **Loaded** : `{len(Archx.manager.loaded_plugins)}`
        ➕ **Enabled** : `{len(Archx.manager.enabled_plugins)}`
        ➖ **Disabled** : `{len(Archx.manager.disabled_plugins)}`
        ❎ **Unloaded** : `{len(Archx.manager.unloaded_plugins)}`

⚔ **Commands** : `{len(Archx.manager.commands)}`
        ✅ **Loaded** : `{len(Archx.manager.loaded_commands)}`
        ➕ **Enabled** : `{len(Archx.manager.enabled_commands)}`
        ➖ **Disabled** : `{len(Archx.manager.disabled_commands)}`
        ❎ **Unloaded** : `{len(Archx.manager.unloaded_commands)}`

⚖ **Filters** : `{len(Archx.manager.filters)}`
        ✅ **Loaded** : `{len(Archx.manager.loaded_filters)}`
        ➕ **Enabled** : `{len(Archx.manager.enabled_filters)}`
        ➖ **Disabled** : `{len(Archx.manager.disabled_filters)}`
        ❎ **Unloaded** : `{len(Archx.manager.unloaded_filters)}`
"""
    elif 'p' in type_:
        if name_:
            if name_ in Archx.manager.plugins:
                plg = Archx.manager.plugins[name_]
...
🗂 **Total** : `{len(Archx.manager.plugins)}`
✅ **Loaded** : `{len(Archx.manager.loaded_plugins)}`
➕ **Enabled** : `{len(Archx.manager.enabled_plugins)}`
➖ **Disabled** : `{len(Archx.manager.disabled_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in Archx.manager.disabled_plugins))}`
❎ **Unloaded** : `{len(Archx.manager.unloaded_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in Archx.manager.unloaded_plugins))}`
"""
    elif 'c' in type_:
        if name_:
            if not name_.startswith(Config.CMD_TRIGGER):
                n_name_ = Config.CMD_TRIGGER + name_
            if name_ in Archx.manager.commands:
                cmd = Archx.manager.commands[name_]
            elif n_name_ in Archx.manager.commands:
                cmd = Archx.manager.commands[n_name_]
            else:
...
🗂 **Total** : `{len(Archx.manager.commands)}`
✅ **Loaded** : `{len(Archx.manager.loaded_commands)}`
➕ **Enabled** : `{len(Archx.manager.enabled_commands)}`
➖ **Disabled** : `{len(Archx.manager.disabled_commands)}`
        `{'`,    `'.join((cmd.name for cmd in Archx.manager.disabled_commands))}`
❎ **Unloaded** : `{len(Archx.manager.unloaded_commands)}`
        `{'`,    `'.join((cmd.name for cmd in Archx.manager.unloaded_commands))}`
"""
    elif 'f' in type_:
        if name_:
            if name_ in Archx.manager.filters:
                flt = Archx.manager.filters[name_]
...
🗂 **Total** : `{len(Archx.manager.filters)}`
✅ **Loaded** : `{len(Archx.manager.loaded_filters)}`
➕ **Enabled** : `{len(Archx.manager.enabled_filters)}`
        `{'`,    `'.join((flt.name for flt in Archx.manager.enabled_filters))}`
➖ **Disabled** : `{len(Archx.manager.disabled_filters)}`
        `{'`,    `'.join((flt.name for flt in Archx.manager.disabled_filters))}`
❎ **Unloaded** : `{len(Archx.manager.unloaded_filters)}`
        `{'`,    `'.join((flt.name for flt in Archx.manager.unloaded_filters))}`
"""
    else:
...
async def enable(message: Message) -> None:
    ...
    type_ = list(message.flags)
    if 'p' in type_:
        found = set(names_).intersection(set(Archx.manager.plugins))
        if found:
            out = await Archx.manager.enable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.commands))
        if found:
            out = await Archx.manager.enable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.filters))
        if found:
            out = await Archx.manager.enable_filters(list(found))
            if out:
...
async def disable(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(Archx.manager.plugins))
        if found:
            out = await Archx.manager.disable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.commands))
        if found:
            out = await Archx.manager.disable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.filters))
        if found:
            out = await Archx.manager.disable_filters(list(found))
            if out:
...
async def load(message: Message) -> None:
    ...
    if message.flags:
        ...
        if 'p' in type_:
            found = set(names_).intersection(set(Archx.manager.plugins))
            if found:
                out = await Archx.manager.load_plugins(list(found))
                if out:
            ...
            found = set(names_).intersection(set(Archx.manager.commands))
            if found:
                out = await Archx.manager.load_commands(list(found))
                if out:
            ...
            found = set(names_).intersection(set(Archx.manager.filters))
            if found:
                out = await Archx.manager.load_filters(list(found))
                if out:
...
async def unload(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(Archx.manager.plugins))
        if found:
            out = await Archx.manager.unload_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.commands))
        if found:
            out = await Archx.manager.unload_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Archx.manager.filters))
        if found:
            out = await Archx.manager.unload_filters(list(found))
            if out:
...
@Archx.on_cmd('reload', about={'header': "Reload all plugins"}, allow_channels=False)
async def reload_(message: Message) -> None:
    """ Reload all plugins """
    await message.edit("`Reloading All Plugins`")
    await message.edit(
        f"`Reloaded {await Archx.reload_plugins()} Plugins`", del_in=3, log=__name__)


@Archx.on_cmd('clear', about={'header': "clear all save filters in DB"}, allow_channels=False)
async def clear_(message: Message) -> None:
    """ clear all save filters in DB """
    await message.edit("`Clearing DB...`")
    await message.edit(
        f"**Cleared Filters** : `{await Archx.manager.clear()}`", del_in=3, log=__name__)
