...
from Archx.utils import runcmd

CHANNEL = Archx.getCLogger()


@Archx.on_cmd("eval", about={
    'header': "run python code line | lines",
    'flags': {'-s': "silent mode (hide STDIN)"},
    'usage': "{tr}eval [flag] [code lines]",
    'examples': [
        "{tr}eval print('Archx')", "{tr}eval -s print('Archx')",
...
async def eval_(message: Message):
    ...
    async def aexec(code):
        head = "async def __aexec(Archx, message, replied, old):\n try:\n  "
        ...
        return await locals()['__aexec'](Archx, message, message.reply_to_message, old)
    try:
...
@Archx.on_cmd("exec", about={
    'header': "run commands in exec",
    'usage': "{tr}exec [commands]",
    'examples': "{tr}exec echo \"Archx\""}, allow_channels=False)
...
@Archx.on_cmd("term", about={
    'header': "run commands in shell (terminal)",
    'usage': "{tr}term [commands]",
    'examples': "{tr}term echo \"Archx\""}, allow_channels=False)
